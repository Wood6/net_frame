...
int CreatDaemon()
{
	...
	{
	case -1:
		LogErrorCore(NGX_LOG_EMERG, errno, "CreatDaemon() 中fork（）失败！");
		return -1;
	case 0:
		// 子进程，直接break跳到后面去执行
		break;
	...
	if (setsid() == -1)
	{
		LogErrorCore(NGX_LOG_EMERG, errno, "CreatDaemon()中setsid()失败！");
		return -1;
	}
	...
	if (-1 == fd)
	{
		LogErrorCore(NGX_LOG_EMERG, errno, "CreatDaemon()中open(\"/dev/null\")失败！");
		return -1;
	}
	...
	if (fd > STDERR_FILENO)  //fd应该是3，这个应该成立
	{
		...
		{
			LogErrorCore(NGX_LOG_EMERG, errno, "ngx_daemon()中close(fd)失败!");
			return -1;
		}
	}

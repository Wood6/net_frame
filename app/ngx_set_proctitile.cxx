 ...
 * 输入参数：(const char *title)
	title 字符串指针，指向要设置成的进程名字字符
...
void SetProcTitle(const char *title)
{
	...
	// (1)计算新标题长度
	size_t len_new_title = strlen(title);

	// (2)计算总的原始的argv那块内存的总长度【包括各种参数也包含环境变量】    
	size_t len_old_title = g_argv_need_mem + g_env_need_mem;  // argv 和 environ 内存总和
	if (len_old_title <= len_new_title)
	{
	...
	strcpy(ptmp, title);
	ptmp += len_new_title; //跳过标题

	// (5)把剩余的原argv以及environ所占的内存全部清0，否则会出现在ps的cmd列可能还会残余一些没有被覆盖的内容；
	size_t len_redundant = len_old_title - len_new_title;  //内存总和减去标题字符串长度(不含字符串末尾的\0)，剩余的大小，就是要memset的；
	memset(ptmp, 0, len_redundant);
